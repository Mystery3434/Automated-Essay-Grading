...
import csv
import pandas as pd
from yourapp.givememyscore import givememyscore
...
# Shortened prompts without background information
p1 = "Write a letter to your local newspaper in which you state your opinion on the effects computers have on people. Persuade the readers to agree with you."
p2 = "Write a persuasive essay to a newspaper reflecting your vies on censorship in libraries. Do you believe that certain materials, such as books, music, movies, magazines, etc., should be removed from the shelves if they are found offensive? Support your position with convincing arguments from your own experience, observations, and/or reading."
p3 = "Write a response that explains how the features of the setting affect the cyclist. In your response, include examples from the essay that support your conclusion."
p4 = "\"When they come back, Saeng vowed silently to herself, in the spring, when the snows melt and the geese return and this hibiscus is budding, then I will take that test again.\" Write a response that explains why the author concludes the story with this paragraph. In your response, include details and examples from the story that support your ideas."
p5 =  "Describe the mood created by the author in the memoir. Support your answer with relevant and specific information from the memoir."
p6 = "Based on the excerpt, describe the obstacles the builders of the Empire State Building faced in attempting to allow dirigibles to dock there. Support your answer with relevant and specific information from the excerpt."
p7 = "Write about patience. Being patient means that you are understanding and tolerant. A patient person experience difficulties without complaining. Do only one of the following: write a story about a time when you were patient OR write a story about a time when someone you know was patient OR write a story in your own way about patience."
p8 = "We all understand the benefits of laughter. For example, someone once said, “Laughter is the shortest distance between two people.” Many other people believe that laughter is an important part of any relationship. Tell a true story in which laughter was one element or part."
#print(len(p1.split()), len(p2.split()), len(p3.split()), len(p4.split()), len(p5.split()), len(p6.split()), len(p7.split()), len(p8.split()))



class BertForSequenceRegression(BertPreTrainedModel):  # Creating a class based on the pre-defined BertForSequenceClassification class
    r"""
...
def test_single_essay(model, tokenizer, prompt, e1):
    tokenized_text = tokenizer.tokenize("[CLS] " + prompt + " [SEP] " + e1 + " [SEP]")
    test_input_ids = [tokenizer.convert_tokens_to_ids(tokenized_text)]

    test_token_type_ids = [[0 if i <= test_input_ids[j].index(102) else 1 for i in range(len(test_input_ids[j]))] for j
                           in range(len(test_input_ids))]

    # Pad our input tokens
    test_token_type_ids = pad_sequences(test_token_type_ids, maxlen=MAX_LEN, dtype="long", truncating="post",
                                        padding="post")
    ...
    model.eval()
    with torch.no_grad():
        return model(test_input.to('cuda'), token_type_ids=test_token_type_ids.to('cuda'),
                     attention_mask=torch.tensor(test_attention_masks).to('cuda'))

def get_data(file_loc):
  '''
  Function to get just the first essay dataset,
  and will update later to be compatible with the other datasets.
  '''
  set1_essays = dict()
...
def main(student_essay, prompt):
    ...
    #tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    tokenizer = BertTokenizer.from_pretrained('/home/flask/app/yourapp/modelcontext')

    MAX_LEN = 384 + 64


    model = BertForSequenceRegression.from_pretrained('/home/flask/app/yourapp/modelcontext',
                                                      num_labels=1)
    model.cuda()
    ...
    score = test_single_essay(model, tokenizer, prompt, student_essay)[0].to('cpu')[0,0]
...
if __name__=="__main__":
    ...
    # Load pre-trained model tokenizer (vocabulary)
    #tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    tokenizer = BertTokenizer.from_pretrained('./modelcontext')

    MAX_LEN = 384


    model = BertForSequenceRegression.from_pretrained('./modelcontext',
                                                      num_labels=1)
    model.cuda()
    param_optimizer = list(model.named_parameters())
    no_decay = ['bias', 'gamma', 'beta']
