...
def fact_check_committee(essay, facts, predictor, model, print_corresponding_facts=True):
    essay_sents = essay.split(".")
    entailment_matrix = generate_entailment_matrix(essay_sents, facts, predictor)
    similarity_matrix = get_similarity_matrix(essay_sents, facts, model)
    fuzzy_matrix = get_fuzzy_matrix(essay_sents, facts)
    does_entail = 0
    is_similar = 0
    has_fuzzy_match = 0
    ...
    for i in range(entailment_matrix.shape[1]):
        for j in range(entailment_matrix.shape[0]):
            if (entailment_matrix[j][i] > 0.5 and similarity_matrix[j][i] > 0.6) or (
                    similarity_matrix[j][i] > 0.6 and fuzzy_matrix[j][i] > 0.5) or (
...
def fact_check_committee_four_methods(essay, facts, stop_words, predictor, model, print_corresponding_facts=True):
    # Predictor refers to the NLI model
    # Model refers to the Universal Sentence Encoding model
...
# module_url = "/home/flask/app/yourapp/universal_sentence_encoder"
# #module_url = "universal_sentence_encoder"
...
def main(essay, facts):
    ...
    module_url = "https://tfhub.dev/google/universal-sentence-encoder/4"  # @param ["https://tfhub.dev/google/universal-sentence-encoder/4", "https://tfhub.dev/google/universal-sentence-encoder-large/5"]
    #module_url = "/home/flask/app/yourapp/universal_sentence_encoder"
    module_url = "/home/flask/app/yourapp/universal_sentence_encoder_transformer"
    #module_url = "universal_sentence_encoder"
    model = hub.load(module_url)
    print("module %s loaded" % module_url)
    ...
    predictor_terminal_path = 'predictor'
    predictor_docker_path = "/home/flask/app/yourapp/predictor"
    predictor = Predictor.from_path(predictor_docker_path)

    stop_words = set(stopwords.words('english'))
    fact_status, fact_count = fact_check_committee(essay, facts, predictor, model)
